...
def parse_maven_metadata(url: str) -> MavenMetadata:
    ...
    return {
        "release": re.search("(?<=<release>).*?(?=</release>)", r.text, re.MULTILINE).group(),  
        "latest": re.search("(?<=<latest>).*?(?=</latest>)", r.text, re.MULTILINE).group(),  
...
def assert_func(expression: bool) -> None:
    ...
    See https://docs.python.org/3/using/cmdline.html?highlight=pythonoptimize
    """
    if not expression:
