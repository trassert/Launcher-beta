...
def install_libraries(id: str, libraries: List[ClientJsonLibrary], path: str, callback: CallbackDict) -> None:
    ...
    for count, i in enumerate(libraries):
        ...
        if native != "":
            download_file(i["downloads"]["classifiers"][native]["url"], os.path.join(current_path, jar_filename_native), callback, sha1=i["downloads"]["classifiers"][native]["sha1"], session=session, minecraft_directory=path)  
...
def install_minecraft_version(versionid: str, minecraft_directory: Union[str, os.PathLike], callback: Optional[CallbackDict] = None) -> None:
    ...
    .. code:: python
        callback = {
            "setStatus": some_function, 
            "setProgress" some_function, 
            "setMax": some_function, 
        }
