...
def parse(string):
    ...
    for match in PATH_REGEXP.finditer(string):
        ...
        token_pattern = (
            parts['capture']
            or parts['group']
            or ('.*' if parts['asterisk'] else '[^%s]+?' % delimiter)
        )
        if not parts['name']:
            parts['name'] = key
            key += 1
...
def tokens_to_template(tokens):
    ...
    def template_function(obj):
        ...
        for token in tokens:
            ...
            path += token['prefix'] + \
                urllib.quote(value.encode('utf8'), '-_.!~*\'()')
        return path
...
def tokens_to_pattern(tokens, end=True, strict=False):
    ...
    for token in tokens:
        if isinstance(token, six.string_types):
            ...
            continue
        parts = {'prefix': escape_string(
            token['prefix']), 'capture': token['pattern'], 'name': ''}
...
def pattern(path, **options):
    ...
    :param strict: Enforce trailing slash in matched strins (default ``False``) :return: A regular expression pattern string
    '''
